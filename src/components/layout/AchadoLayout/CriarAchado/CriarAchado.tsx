...
export default function CriarAchado() {
    ...
    const SchemaCriarContaEstudante = object({
        ...
        filiacao: string().required("Filiacao é um campo obrigatório").min(4, "A filiacao passe deve ter pelo menos 4 caracteres").max(100, "A filiacao passe deve ter no máximo 100 caracteres"),
        descricao: string().required("Descricao é um campo obrigatório").min(20, "A descricao passe deve ter pelo menos 4 caracteres").max(300, "A descricao passe deve ter no máximo 300 caracteres"),
        ...
        <div className=''>
            <div className='bg-indigo-100 flex items-center justify-center h-full'>
                ...
                <section className="w-[100%] p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
                    <h1 className="text-xl font-bold text-black capitalize dark:text-white">CRIAR OBJECTO ACHADO</h1>
                        ...
                        onSubmit={async values => {
                            ...
                            dataUtilizador.map(async ({ id, email }: { id: string, email: string }) => {
                                ...
                                if (email === emailUtilizador) {
                                    ...
                                    const result_api = await api.post(`/${rota}/criar_documento_achado_${rota}/${id}`,
                                        formData
                                        ,
                                        {
                                            headers: {
                                    ...
                                    if (result_api.data) {
                                        ...
                                        setTimeout(() => {
                                            ...
                                            return navigate(`/suchen/${rota}/achado`)
