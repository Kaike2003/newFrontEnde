...
function BilheteIdentidadePerdido() {
    ...
    const callBack = useCallback(async () => {
        ...
        const response = await api.get("/publico/listar_todos_bilhete_identidade_perdidos_limite/6")
    ...
    async function fetchData() {
        ...
        const response = await api.get(`/publico/listar_todos_bilhete_identidade_perdidos_limite/${limite}`)
        const newData = response.data
        setData(newData)
    ...
    return (
        ...
        >
            ...
            >
                {data.map(({ id, aprovado, foto, descricao, entregue, nome, publicado, perdido }: IData) => {
...
export default BilheteIdentidadePerdido